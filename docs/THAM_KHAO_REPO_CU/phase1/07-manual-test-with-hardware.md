# Manual Test with Hardware — TTV Studio App
...
- App đang chạy: `python -m ttv_studio` (hoặc `uv run python -m ttv_studio`).
...
| H-08 | QR provision scan          | 1. Có file PNG QR hợp lệ (schema `ttv-studio-provision/v1`). 2. Add Logger → Scan QR. 3. Chọn file. | Các field: host, token, apiPort được điền tự động.                                                  |                                                                             |      |
